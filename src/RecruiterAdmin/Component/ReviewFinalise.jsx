  ...
  const displayQuestions = questions.map((q, idx) => {
    ...
    if (q.type === 'mcq') {
      ...
      return {
        ...
        expected_answer: content.expected_answer || content.reference_solution || '',
        complexity_constraints: content.complexity_constraints || '',
        tags: [q.skill], skills: [q.skill],
        time: q.time_limit || 300, difficulty: q.difficulty || 'medium',
        questionType: 'Coding', marks: q.positive_marking || 2,
        type: q.type, skill: q.skill
          ...
          <div key={question.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            ...
            {(question.expected_answer || (question.questionType === 'MCQ' && question.explanation)) && (
              <div className="rounded-xl px-4 py-3 mb-2 text-sm" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                ...
                <div className="text-green-800 mt-1">
                  {question.questionType === 'Coding' ? (
                    <pre className="bg-green-900/5 p-3 rounded-lg border border-green-100 font-mono text-xs overflow-x-auto">
                      <code>{question.expected_answer || 'No reference solution provided'}</code>
                    </pre>
                  ) : (
                    <span>{question.expected_answer || question.explanation}</span>
                  )}
                </div>
              </div>
            )}
            ...
            {question.questionType === 'Coding' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                {question.input_spec && (
                  <div className="text-[11px] bg-gray-50 border border-gray-100 rounded-xl p-3">
                    <strong className="text-gray-500 uppercase tracking-tight block mb-1">Input Specification</strong>
                    <span className="text-gray-700">{question.input_spec}</span>
                  </div>
                )}
                {question.output_spec && (
                  <div className="text-[11px] bg-gray-50 border border-gray-100 rounded-xl p-3">
                    <strong className="text-gray-500 uppercase tracking-tight block mb-1">Output Specification</strong>
                    <span className="text-gray-700">{question.output_spec}</span>
                  </div>
                )}
                {question.complexity_constraints && (
                  <div className="sm:col-span-2 text-[11px] bg-amber-50/50 border border-amber-100/50 rounded-xl p-3">
                    <strong className="text-amber-600 uppercase tracking-tight block mb-1">Complexity Constraints</strong>
                    <span className="text-amber-800 font-mono">{question.complexity_constraints}</span>
                  </div>
                )}
              </div>
            )}
